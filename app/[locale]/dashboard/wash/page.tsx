...
import { mockWashBays } from '@/lib/mock/data';
import { cn } from '@/lib/utils/cn';
import type { WashBay, WashStatus } from '@/types';
...
function BayCard({ bay }: { bay: WashBay }) {
    ...
    >
      ...
      {bay.status === 'washing' && (
        <motion.div
          ...
          className="absolute inset-0 bg-gradient-to-r from-transparent via-blue-400/5 to-transparent pointer-events-none"
        />
      )}
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span className={cn('opacity-60', cfg.badge.split(' ')[1])}>
          {cfg.icon}
        </span>
      </div>
      ...
      {bay.carPlate ? (
        <div className="space-y-1.5">
          ...
          <div className="text-xs text-zinc-400">{bay.carBrand}</div>
          {bay.startedAt && (
            <div className="flex items-center gap-1 text-xs text-zinc-500">
              <Clock className="w-3 h-3" />
              Boshlandi: {bay.startedAt}
            </div>
          )}
          {bay.bookedAt && (
            <div className="flex items-center gap-1 text-xs text-zinc-500">
              <Clock className="w-3 h-3" />
              Bron vaqti: {bay.bookedAt}
            </div>
          )}
        </div>
      ) : (
        <div className="text-sm text-zinc-600 italic">Mashina yo'q</div>
      )}
...
export default function WashPage() {
  const [bays] = useState(mockWashBays);
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div>
          <h2 className="text-lg font-bold text-white">Yuvish Joyi</h2>
          <p className="text-xs text-zinc-500">{bays.length} ta joy — real vaqt holati</p>
        </div>
        <button className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-300 text-xs font-medium transition-colors">
