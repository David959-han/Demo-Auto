...
export default function CustomersPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div>
          <h2 className="text-lg font-bold text-white">Mijozlar</h2>
          <p className="text-xs text-zinc-500">{mockCustomers.length} ta mijoz ro'yxatda</p>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="hidden sm:flex items-center gap-6 text-xs text-right">
                  <div>
                    <div className="text-zinc-400 font-medium">{cars.length} ta</div>
                    <div className="text-zinc-600">avto</div>
                  </div>
                  <div>
                    <div className="text-zinc-400 font-medium">{orders.length} ta</div>
                    <div className="text-zinc-600">buyurtma</div>
                  </div>
                  <div>
                    <div className="text-amber-400 font-mono font-bold">
                      {totalSpent > 0 ? `${(totalSpent / 1000).toFixed(0)}K` : '—'}
                    </div>
                    <div className="text-zinc-600">jami</div>
                  </div>
                </div>
              ...
              {isOpen && (
                ...
                >
                  <div className="text-xs text-zinc-500 font-medium mb-3 flex items-center gap-1">
                    <Car className="w-3 h-3" /> Avtomobillari
                  </div>
                  <div className="space-y-2">
                    {cars.map((car) => (
                  ...
                  {orders.length > 0 && (
                    <div className="mt-4">
                      <div className="text-xs text-zinc-500 font-medium mb-2">So'nggi buyurtmalar</div>
                      <div className="space-y-1.5">
                        {orders.slice(0, 3).map((o) => (
                          <div key={o.id} className="flex items-center justify-between text-xs text-zinc-400">
                            ...
                            <span className="truncate mx-3 max-w-[160px]">{o.description}</span>
